...
from expos_pdv.config.common.settings import SURVEY_SERVICE_BASE_URL, SQL_PATH
from expos_pdv.config.reset_rejected_surveys.settings import RRS_DAG_START_DATE_VALUE, RRS_DAG_SCHEDULE_INTERVAL, \
    RRS_DAG_ID
...
class ResetRejectedSurveysDagFactory:
    ...
    def get_surveys_to_reset():
        with open(f'{SQL_PATH}/reset_rejected_surveys/get_surveys_to_reset.sql', 'r') as file:
            sql = file.read()
    ...
    def update_surveys(ti):
        ...
        if not to_update:
            return
        with open(f'{SQL_PATH}/reset_rejected_surveys/update_reset_surveys.sql', 'r') as file:
            sql = file.read()
            parameterized_query(
